...
## V2.4 访客界面去平台字样 + IP 深度评分自动执行

访客页面：
- 去除 GitHub / Cloudflare / Worker Full Stack 等架构与平台字样
- 页脚仅显示“鑫淼网络检测 · V2.4”
- DNS 页面不再暴露底层部署方式
- 保留必要的第三方服务名称，例如 Cloudflare DoH、Google DoH、站点连通性测试名称；这些属于检测对象，不属于部署架构展示

IP 深度评分：
- 进入页面后自动读取当前公网 IP
- 自动触发现有评分逻辑
- 不需要再次输入当前 IP 或点击评分按钮
- 仍可保留自定义 IP 查询能力（如原页面存在）


## V2.4.1 网络连通自动测试

- 点击顶部“网络连通”后立即自动执行全部网站连通测试
- 不需要再点击“开始测试”
- 页面原测试按钮改为“重新测试”
...
## V2.5.4 首页布局调整
- “鑫淼的世界 / yhqtv.com”官方网站入口保持首页上方
- “三线路出口 IP”整体移动到官方网站卡片正下方
- 三线路检测逻辑、国家地区双显、IP 隐藏功能保持不变

## V2.5.5 IP 隐私按钮布局
- “隐藏 50% IP”移动到“三线路出口 IP”标题右侧
- 手机端自动缩小；极窄屏自动换到标题下一行
- 原隐藏逻辑保持不变
